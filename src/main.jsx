...
function detectDeviceModel(text) {
  ...
  if (modelLine) {
    ...
    if (identifier) {
      return {
        ...
        model: IPHONE_MODELS[identifier[0]] || 'iPhone identificado fora da base cadastrada',
        confidence: 'Média'
      };
    }
  ...
  if (marketing) {
    return {
      identifier: 'Modelo comercial informado no log',
      model: marketing[0].replace(/\s+/g, ' '),
      confidence: 'Média'
    };
  }

  return {
    identifier: 'Não encontrado no log',
...
function App() {
  ...
  function handleFile(event) {
    ...
    reader.onerror = () => {
      setFileError('Não foi possível ler o arquivo. Tente copiar e colar o conteúdo do log manualmente.');
    };
  ...
  return (
    <main className="app">
      <section className="hero">
        <div className="badge">
          <Smartphone size={18} />
          Assistência técnica inteligente
        </div>

        <h1>PanicLog Analyzer</h1>

        <p>
          Triagem inicial de logs Panic Full de iPhone por palavras-chave, prioridade e recomendações técnicas.
        </p>
      </section>
      ...
      <section className="grid">
        <div className="card input-card">
          ...
          <div className="actions">
            ...
            <button className="button" onClick={() => setLog(sampleLog())}>
        ...
        <div className="card result-card">
          ...
          {!log.trim() ? (
            <div className="empty">
              Cole um log para iniciar a análise.
            </div>
          ) : (
            <>
              <div className={`priority ${result.priority.toLowerCase()}`}>
                Prioridade: {result.priority}
              </div>

              <p className="muted">
                Caracteres analisados: {result.totalChars}
              </p>

              <div className="device-box">
                <strong>Modelo do aparelho:</strong> {result.device.model}
                <br />
                <small>
                  Identificador: {result.device.identifier} • Confiança: {result.device.confidence}
                </small>
              </div>
              ...
              {result.matches.length === 0 ? (
                <div className="alert">
                  <AlertTriangle />
                  Nenhum padrão cadastrado foi identificado. Faça análise manual do panicString.
                </div>
              ) : (
                result.matches.map((match, index) => (
                  <div className="match" key={index}>
                    ...
                    <p>
                      <strong>Termos encontrados:</strong> {match.foundTerms.join(', ')}
                    </p>

                    <p>
                      <strong>Diagnóstico provável:</strong> {match.diagnosis}
                    </p>

                    <p>
                      <strong>Recomendação:</strong> {match.recommendation}
                    </p>
                  </div>
                ))
              )}

              <button className="button full" onClick={saveResult}>
      ...
      <section className="card history-card">
        ...
        <button className="button danger small" onClick={clearHistory}>
          Apagar histórico
        </button>
      </section>

      <footer>
        LOUTECH • Projeto acadêmico de Engenharia de Software • Vercel
      </footer>
    </main>
  );
}
